...
namespace SenseNet.Portal.Portlets.ContentHandlers
{
    ...
    public class SurveyListItem : GenericContent
    {
        ...
        public override void Save(NodeSaveSettings settings)
        {
            ...
            var parentForm = this.LoadContentList() as SurveyList;
            if (isNew && !import && parentForm != null && parentForm.OnlySingleResponse)
            {
                ...
                if (parentForm.IsFilled())
                    throw new InvalidOperationException(SR.GetString(SR.Exceptions.Survey.OnlySingleResponse, User.Current.Username));
            }
                        ...
                        try
                        {
                            ...
                            if (stream != null)
                            {
                                ...
                                if (string.IsNullOrEmpty(fileName))
                                    fileName = string.Format("{0}-{1}", this.Name, field.Name.Replace("#", "-"));
        ...
        {
            return string.Format("\nName: {0}\n\nUser: {1}\n\n\r\n{2}", Name, CreatedBy.Name, CreateEmailBody(false));
        }
